...
//	printf( "\x1b[32m\x1b[1m%s\x1b[39m\x1b[0m", str );
	fprintf( stderr, "\x1b[32m\x1b[1m%s\x1b[39m\x1b[0m", str );
}
void Rprint( const char *str )
{
//	printf( "\033[1m\033[31m%s\033[30m\033[0m", str );
	fprintf( stderr, "\033[1m\033[31m%s\033[30m\033[0m", str );
}
...
static int isValidFileName( const char* filename, const char* ext )
{
	...
	for( int i = 0 ; i < ext_len ; i++ ){
		str[ i ] = filename[ len-3+i ];
//		printf( "%c\n", filename[ len-ext_len+i ] );
	}
	if( strncmp( ext, str, ext_len ) ){
		fprintf( stderr, "Cannot make .%s file (%s)\n", ext, filename );
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}
